...
{
    platform_hnd_t file_handle     = platform_file_open(file_path, PLATFORM_FILE_FLAGS_read | PLATFORM_FILE_FLAGS_share_r);
    platform_file_info_t file_info = platform_file_info_get(file_handle);
    ...
    if (magic != GLTF_MAGIC)
    {
        return (gltf_data_t){0};
    }

    parser.position = 12;

    gltf_json_t json_data = {0};
...
internal gltf_json_t gltf_parse_chunk_json(gltf_parser_t* parser, u32 chunk_length)
{
    ...
    gltf_json_t result = {0};

    while (current != NULL)
    {
        buffer_t label_nodes  = buffer_from_cstr("nodes");
        buffer_t label_meshes = buffer_from_cstr("meshes");
        buffer_t label_access = buffer_from_cstr("accessors");
        buffer_t label_views  = buffer_from_cstr("bufferViews");
        buffer_t label_buffer = buffer_from_cstr("buffers");
        buffer_t label_tex    = buffer_from_cstr("textures");
        buffer_t label_img    = buffer_from_cstr("images");
        buffer_t label_sample = buffer_from_cstr("samplers");
        buffer_t label_mat    = buffer_from_cstr("materials");

        if (buffer_is_equal(&label_nodes, &current->label))
        {
            i32 node_count    = json_child_count(current);
            ...
            for (i32 i = 0; i < node_count && node != NULL; i++)
            {
                ...
                //b32 name_is_valid   = json_child_value(parser->arena, node, JSON_VALUE_TYPE_str, _, "name");
                ...
                json_entry_t* children_node = json_child_find(node, &children_label);

                i32* children = NULL;
                if (children_node != NULL)
                {
                    child_count       = json_child_count(children_node);
        ...
        {
            i32 mesh_count    = json_child_count(current);
            ...
            for (i32 i = 0; i < mesh_count && mesh != NULL; i++)
            {
                ...
                json_entry_t* current_prim = json_child_find(mesh, &primitives_buffer);
                i32 primitive_count        = json_child_count(current_prim);

                result.meshes[i].primitives      = MEMORY_PUSH(parser->arena, gltf_primitive_t, primitive_count);
                ...
                for (i32 j = 0; j < primitive_count; j++)
                {
                    ...
                    json_entry_t* attributes = json_child_find(current_prim, &attributes_buffer);
                    ...
                    b32 texcoord0_is_valid = json_child_value(parser->arena, attributes, JSON_VALUE_TYPE_i32, &texcoord0, "TEXCOORD_0");
                    b32 texcoord1_is_valid = json_child_value(parser->arena, attributes, JSON_VALUE_TYPE_i32, &texcoord1, "TEXCOORD_1");
                    ...
                    result.meshes[i].primitives[j].texcoord0 = texcoord0_is_valid ? texcoord0 : -1;
                    result.meshes[i].primitives[j].texcoord1 = texcoord1_is_valid ? texcoord1 : -1;
        ...
        {
            i32 accessor_count    = json_child_count(current);
        ...
        {
            i32 view_count           = json_child_count(current);
        ...
        {
            i32 buffer_count    = json_child_count(current);
        ...
        {
            i32 texture_count    = json_child_count(current);
            result.textures      = MEMORY_PUSH(parser->arena, gltf_texture_t, texture_count);
            result.texture_count = texture_count;

            json_entry_t* tex = current->child;
        ...
        {
            i32 image_count    = json_child_count(current);
            result.images      = MEMORY_PUSH(parser->arena, gltf_image_t, image_count);
            result.image_count = image_count;

            json_entry_t* image = current->child;
            for (i32 i = 0; i < image_count && image != NULL; i++)
            {
                ...
                buffer_t png      = buffer_from_cstr("image/png");
                buffer_t jpeg     = buffer_from_cstr("image/jpeg");
                buffer_t mime_buf = buffer_from_cstr(mime_type);
        ...
        {
            i32 sampler_count    = json_child_count(current);
            result.samplers      = MEMORY_PUSH(parser->arena, gltf_sampler_t, sampler_count);
            result.sampler_count = sampler_count;

            json_entry_t* sampler = current->child;
        ...
        {
            i32 material_count    = json_child_count(current);
            result.materials      = MEMORY_PUSH(parser->arena, gltf_material_t, material_count);
            result.material_count = material_count;

            buffer_t buffer_pbr     = buffer_from_cstr("pbrMetallicRoughness");
            buffer_t buffer_tex_clr = buffer_from_cstr("baseColorTexture");
            buffer_t buffer_tex_mr  = buffer_from_cstr("metallicRoughnessTexture");
            buffer_t buffer_tex_nm  = buffer_from_cstr("normalTexture");
            buffer_t buffer_tex_ao  = buffer_from_cstr("occlusionTexture");
            buffer_t buffer_tex_em  = buffer_from_cstr("emissiveTexture");

            json_entry_t* mat = current->child;
            for (i32 i = 0; i < material_count && mat != NULL; i++)
            {
                ...
                json_entry_t* pbr = json_child_find(mat, &buffer_pbr);
                ...
                if (pbr != NULL)
                {
                    json_entry_t* tex_clr = json_child_find(pbr, &buffer_tex_clr);
                    json_entry_t* tex_mr  = json_child_find(pbr, &buffer_tex_mr);
                ...
                json_entry_t* tex_nm = json_child_find(mat, &buffer_tex_nm);
                ...
                json_entry_t* tex_ao = json_child_find(mat, &buffer_tex_ao);
                ...
                json_entry_t* tex_em = json_child_find(mat, &buffer_tex_em);
                ...
                {
                    buffer_t buf_mask     = buffer_from_cstr("MASK");
                    buffer_t buf_blend    = buffer_from_cstr("BLEND");
                    buffer_t buf_mode_val = buffer_from_cstr(alpha_mode);
...
internal gltf_data_t gltf_parse_chunk_binary(gltf_parser_t* parser, u32 chunk_length, gltf_json_t* json_data)
{
    ...
    result.transforms        = MEMORY_PUSH_ZERO(parser->arena, mat4_t, json_data->node_count);
    result.children          = MEMORY_PUSH_ZERO(parser->arena, i32*, json_data->node_count);
    result.children_count    = MEMORY_PUSH_ZERO(parser->arena, i32, json_data->node_count);
    result.parents           = MEMORY_PUSH_ZERO(parser->arena, i32, json_data->node_count);
    result.mesh_ids          = MEMORY_PUSH_ZERO(parser->arena, i32, json_data->node_count);
    result.mesh_offsets      = MEMORY_PUSH_ZERO(parser->arena, i32, json_data->mesh_count);
    result.mesh_primitives   = MEMORY_PUSH_ZERO(parser->arena, i32, json_data->mesh_count);
    result.mesh_has_tangents = MEMORY_PUSH_ZERO(parser->arena, b8, json_data->primitive_count);
    result.meshes            = MEMORY_PUSH_ZERO(parser->arena, mesh_t, json_data->primitive_count);
    result.textures          = MEMORY_PUSH_ZERO(parser->arena, texture_t, json_data->texture_count);
    result.materials         = MEMORY_PUSH_ZERO(parser->arena, material_t, json_data->material_count);
    ...
    {
        gltf_texture_t* tex     = &json_data->textures[texture_idx];
        gltf_sampler_t* sampler = &json_data->samplers[tex->sampler];
    ...
    for (i32 mesh_idx = 0; mesh_idx < json_data->mesh_count; mesh_idx++)
    {
        ...
        for (i32 prim_idx = 0; prim_idx < json_data->meshes[mesh_idx].primitive_count; prim_idx++)
        {
            ...
            i32 acs_tan = json_data->meshes[mesh_idx].primitives[prim_idx].tangent;
            i32 acs_uv0 = json_data->meshes[mesh_idx].primitives[prim_idx].texcoord0;
            i32 acs_uv1 = json_data->meshes[mesh_idx].primitives[prim_idx].texcoord1;
            ...
            i32 data_offset_uv0 = json_data->buffer_views[json_data->accessors[acs_uv0].buffer_view_id].byte_offset
                                + json_data->accessors[acs_uv0].byte_offset;
            ...
            EMBER_ASSERT(
                ...
                (json_data->accessors[acs_pos].count == json_data->accessors[acs_uv0].count)
            );
            ...
            result.meshes[mesh_offset].vertices = MEMORY_PUSH(parser->arena, vertex_t, result.meshes[mesh_offset].vertex_count);
            ...
            gltf_parse_components(
                (data + data_offset_uv0),
                result.meshes[mesh_offset].vertices,
                result.meshes[mesh_offset].vertex_count,
                offsetof(vertex_t, uv0),
                sizeof(vertex_t),
                json_data->accessors[acs_uv0].component_type,
                json_data->accessors[acs_uv0].type
            );

            if (acs_tan >= 0)
            {
                i32 data_offset_tan = json_data->buffer_views[json_data->accessors[acs_tan].buffer_view_id].byte_offset
                                    + json_data->accessors[acs_tan].byte_offset;

                gltf_parse_components(
                    (data + data_offset_tan),
                    result.meshes[mesh_offset].vertices,
                    result.meshes[mesh_offset].vertex_count,
                    offsetof(vertex_t, tangent),
                    sizeof(vertex_t),
                    json_data->accessors[acs_tan].component_type,
                    json_data->accessors[acs_tan].type
                );

                result.mesh_has_tangents[mesh_offset] = EMBER_TRUE;
            }

            if (acs_uv1 >= 0)
            {
                i32 data_offset_uv1 = json_data->buffer_views[json_data->accessors[acs_uv1].buffer_view_id].byte_offset
                                    + json_data->accessors[acs_uv1].byte_offset;

                gltf_parse_components(
                    (data + data_offset_uv1),
                    result.meshes[mesh_offset].vertices,
                    result.meshes[mesh_offset].vertex_count,
                    offsetof(vertex_t, uv1),
                    sizeof(vertex_t),
                    json_data->accessors[acs_uv1].component_type,
                    json_data->accessors[acs_uv1].type
                );
            }
            ...
            {
                result.meshes[mesh_offset].vertices[i].color = (vec3_t){1.0f, 1.0f, 1.0f};
            }
...
void gltf_free(gltf_data_t* gltf)
{
